...
class Inceptionv3:
    ...
    def _build_graph(self):
        with tf.variable_scope('main'):
            with tf.variable_scope('stem'):
                ...
                conv2_1 = self._conv_bn_activation(pool1, 80, 3, 1, 'valid', 'conv2_1')
                self.output_shape = self._compute_output_shape(self.output_shape, 3, 'valid', 1)
                conv2_2 = self._conv_bn_activation(conv2_1, 192, 3, 2, 'valid', 'conv2_2')
                self.output_shape = self._compute_output_shape(self.output_shape, 3, 'valid', 2)
                conv2_3 = self._conv_bn_activation(conv2_2, 288, 3, 1, 'valid', 'conv2_3')
                self.output_shape = self._compute_output_shape(self.output_shape, 3, 'valid', 1)
        ...
        with tf.variable_scope('auxilary'):
            ...
            auxiliary_conv1x1 = self._conv_bn_activation(auxiliary_avgpool, 128, 1, 1, 'same', 'auxiliary_conv1x1')
    ...
    def _inception_block1(self, bottom, filters, scope):
        with tf.variable_scope(scope):
            with tf.variable_scope('branch_pool1x1'):
                ...
                branch_pool1x1 = self._conv_bn_activation(branch_pool1x1, filters[3], 1, 1, 'same', 'conv_bn_relu')
            with tf.variable_scope('branch_1x1'):
                branch_1x1 = self._conv_bn_activation(bottom, filters[0], 1, 1, 'same', 'conv_bn_relu')
            with tf.variable_scope('branch_1x1x3x3'):
                branch_1x1x3x3 = self._conv_bn_activation(bottom, filters[1], 1, 1, 'same', 'conv1x1')
                branch_1x1x3x3 = self._conv_bn_activation(branch_1x1x3x3, filters[0], 3, 1, 'same', 'conv3x3')
            with tf.variable_scope('branch_1x1x5x5'):
                branch_1x1x5x5 = self._conv_bn_activation(bottom, filters[0], 1, 1, 'same', 'conv1x1')
                branch_1x1x5x5 = self._conv_bn_activation(branch_1x1x5x5, filters[2], 3, 1, 'same', 'conv3x3_1')
                branch_1x1x5x5 = self._conv_bn_activation(branch_1x1x5x5, filters[2], 3, 1, 'same', 'conv3x3_2')
    ...
    def _grid_size_reduction1(self, bottom, scope):
        with tf.variable_scope(scope):
            with tf.variable_scope('branch_3x3'):
                branch_3x3 = self._conv_bn_activation(bottom, 384, 3, 2, 'same', 'conv3x3')
            with tf.variable_scope('branch_1x1x5x5'):
                ...
                branch_1x1x5x5 = self._conv_bn_activation(branch_1x1x5x5, 96, 3, 2, 'same', 'conv5x5_2')
    ...
    def _inception_block2(self, bottom, filters, scope):
        with tf.variable_scope(scope):
            with tf.variable_scope('branch_1x1'):
                branch_1x1 = self._conv_bn_activation(bottom, filters[0], 1, 1, 'same', 'conv1x1')
            with tf.variable_scope('branch_1x1x7x7'):
                branch_1x1x7x7 = self._conv_bn_activation(bottom, filters[1], 1, 1, 'same', 'conv1x1')
                branch_1x1x7x7 = self._conv_bn_activation(branch_1x1x7x7, filters[1], [1, 7], 1, 'same', 'conv1x7')
                branch_1x1x7x7 = self._conv_bn_activation(branch_1x1x7x7, filters[0], [7, 1], 1, 'same', 'conv7x1')
            with tf.variable_scope('branch_1x1x7x7x7x7'):
                branch_1x1x7x7x7x7 = self._conv_bn_activation(bottom, filters[1], 1, 1, 'same', 'conv1x1')
                branch_1x1x7x7x7x7 = self._conv_bn_activation(branch_1x1x7x7x7x7, filters[1], [7, 1], 1, 'same', 'conv7x1_1')
                branch_1x1x7x7x7x7 = self._conv_bn_activation(branch_1x1x7x7x7x7, filters[1], [1, 7], 1, 'same', 'conv1x7_1')
                branch_1x1x7x7x7x7 = self._conv_bn_activation(branch_1x1x7x7x7x7, filters[1], [7, 1], 1, 'same', 'conv7x1_2')
                branch_1x1x7x7x7x7 = self._conv_bn_activation(branch_1x1x7x7x7x7, filters[0], [1, 7], 1, 'same', 'conv1x7_2')
            with tf.variable_scope('branch_pool1x1'):
                branch_pool1x1 = self._avg_pooling(bottom, 3, 1, 'same', 'pool')
                branch_pool1x1 = self._conv_bn_activation(branch_pool1x1, filters[0], 1, 1, 'same', 'conv1x1')
            axes = 3 if self.data_format == 'channels_last' else 1
    ...
    def _grid_size_reduction2(self, bottom, scope):
        with tf.variable_scope(scope):
            with tf.variable_scope('branch_1x1x3x3'):
                ...
                branch_1x1x3x3 = self._conv_bn_activation(branch_1x1x3x3, 320, 3, 2, 'same', 'conv3x3')
            with tf.variable_scope('branch_1x1x7x7x3x3'):
                branch_1x1x7x7x3x3 = self._conv_bn_activation(bottom, 192, 1, 1, 'same', 'conv1x1')
                branch_1x1x7x7x3x3 = self._conv_bn_activation(branch_1x1x7x7x3x3, 192, [1, 7], 1, 'same', 'conv1x7')
                branch_1x1x7x7x3x3 = self._conv_bn_activation(branch_1x1x7x7x3x3, 192, [7, 1], 1, 'same', 'conv7x1')
                branch_1x1x7x7x3x3 = self._conv_bn_activation(branch_1x1x7x7x3x3, 192, 3, 2, 'same', 'conv3x3')
    ...
    def _inception_block3(self, bottom, filters, scope):
        with tf.variable_scope(scope):
            ...
            with tf.variable_scope('branch_1x1x3x3'):
                branch_1x1x3x3 = self._conv_bn_activation(bottom, filters[1], 1, 1, 'same', 'conv1x1')
                branch_1x1x3x3x1x3 = self._conv_bn_activation(branch_1x1x3x3, filters[1], [1, 3], 1, 'same', 'conv1x3')
                branch_1x1x3x3x3x1 = self._conv_bn_activation(branch_1x1x3x3, filters[1], [3, 1], 1, 'same', 'conv3x1')
                axes = 3 if self.data_format == 'channels_last' else 1
                branch_1x1x3x3 = tf.concat([branch_1x1x3x3x1x3, branch_1x1x3x3x3x1], axis=axes)
            with tf.variable_scope('branch_1x1x3x3x3x3'):
                branch_1x1x3x3x3x3 = self._conv_bn_activation(bottom, filters[2], 1, 1, 'same', 'conv1x1')
                branch_1x1x3x3x3x3 = self._conv_bn_activation(branch_1x1x3x3x3x3, filters[1], 3, 1, 'same', 'conv3x3')
                branch_1x1x3x3x3x3x1x3 = self._conv_bn_activation(branch_1x1x3x3x3x3, filters[1], [1, 3], 1, 'same', 'conv1x3')
                branch_1x1x3x3x3x3x3x1 = self._conv_bn_activation(branch_1x1x3x3x3x3, filters[1], [3, 1], 1, 'same', 'conv3x1')
                branch_1x1x3x3x3x3 = tf.concat([branch_1x1x3x3x3x3x1x3, branch_1x1x3x3x3x3x3x1], axis=axes)
            with tf.variable_scope('branch_pool1x1'):
                branch_pool1x1 = self._avg_pooling(bottom, 3, 1, 'same', 'pool')
                branch_pool1x1 = self._conv_bn_activation(branch_pool1x1, filters[3], 1, 1, 'same', 'conv1x1')
